...
class FeedbackManager {
    ...
    applyAudioFeedback() {
		...
		const colorful = this.smoothstep(0.0, 8.0, data[2] / 255);
		...
		const changerate = this.smoothstep(0.0, 9.0, data[1] / 255);
        const qVal = 1.0 + changerate * gainByEQ;
        this.targets.filter.Q.setTargetAtTime(qVal, now, rampTime);
		
        // A -> Distortion 畫面變化快慢
		const leftRight = this.smoothstep(0.0, 7.0, data[3] / 255.0);
