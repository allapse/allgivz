...
void main() {
	...
	float timeswing2 = u_time * (1.0 + 0.118 * sin(u_time * 2.0 * PI * u_fps * u_bpm * length(vec3(uv, v_z))));
	
	if(abs(uv.x) > 0.618 && abs(uv.y) > 0.618 && length(uv) > 1.618)
	timeswing = mix(timeswing, timeswing2, v_z);
	...
	float v_z_st = 0.0;
	float lrpunch = 0.382 + (u_left - u_right) * 6.18 * punch;
