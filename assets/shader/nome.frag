...
float drawBraille(vec2 pos, vec2 base, float r, int code) {
    ...
    for(int i=0; i<6; i++){
        if(((code >> i) & 1) == 1){
			float pulse = 0.618 + 0.382 * sin(u_time * 1.618 + float(i));
			float dynamicR = r * (1.0 + 0.618 * u_volume);
			vec2 jitter = vec2(sin(u_time * 3.82 + float(i)), cos(u_time * 1.618 + float(i))) * 0.0228;
...
float map_wave(vec3 p) {
    ...
    float freq = 1.618 + u_complexity * 3.82; 
    float amp = 0.0618 + u_complexity * 0.618;

    float wave = sin(mirrorAngle * freq + u_time) * cos(zLoop * 1.618);
    
    float noise = hash(floor(p.z * 3.82)) * u_complexity * 1.618;
...
float fbm(vec2 p) {
    float v = 0.0;
    float amp = 0.618;
    int iter = int(1.618 + u_complexity * 16.18);
    for (int i = 0; i < 6; i++) {
        if (i >= iter) break;
        v += noise(p) * amp;
        p *= 2.28;
...
vec2 refract_logic(vec2 uv, float t, float tension) {
    ...
    for(float i = 1.0; i < 7.0 + 7.0 * u_complexity; i++) {
        p.x += 0.382 / i * sin(i * 3.82 * p.y + t + (u_left + 0.618 * u_volume_smooth) * 0.618 * tension);
        p.y += 0.618 / i * cos(i * 6.18 * p.x + t + (u_right + 0.618 * u_volume) * 0.382 * tension);
    }
    return p;
}

float primeWave(vec3 p, vec3 center, float time) {
    float dist = sin(length(p - center)) / (0.618 + 0.382 * cos(dot(p, center)));
    // 波動：cosine + 衰減
...
void main() {
	...
	// singularity
	uv = abs(uv) - sin(u_left + 0.618 * u_complexity) - cos(u_right + 0.618 * u_intensity);
	...
	// possibility
	vec2 warped = uv;
    warped.x += (0.228 + 0.618 * u_speed) * sin(u_time * 0.382 + uv.y * 6.18 + v_z * 0.618);
    warped.y += (0.618 + 0.228 * u_complexity) * cos(u_time * 0.618 + uv.x * 3.82 + v_z * 0.618);
	...
	if(v_uv.x < v_uv.y + (u_left - u_right) * 6.18 * punch) {
		...
		pos *= 1.0 + u_right * 3.82;
	}
	
	// gliding
	float stripes = pow(sin(abs(atan(pos.y, pos.x)) * floor(6.18 + pow(u_complexity, 3.0) * 6.18)) * sin(1.0 / (sqrt(length(pos) * length(pos) + 0.01)) + u_time * 1.618), 3.0);
	// fbm
    float pattern = pow(fbm(pos + vec2(fbm(pos * 0.618 + u_time), fbm(pos * 0.618 - u_time)) + u_volume_smooth * 0.118), 3.0);
	// erosion
    ...
    vec2 cr = vec2(0.0);
    cr.x = pow(fbm(uv + 1.0 * cq + vec2(1.618, 6.18) + 0.1618 * u_time), 3.0);
    cr.y = pow(fbm(uv + 1.0 * cq + vec2(6.18, 3.82) + 0.118 * u_time), 3.0);
	...
	// regret
	mat3 rrot = rotateY(u_orient.x + u_time * 0.618) * rotateX(u_orient.y + u_time * 0.228);
	
    color *= sym * motion * pow(0.228 + 0.618 * u_intensity, 3.0) / (0.228 + 0.618 * pattern) * stripes / (0.228 + 0.618 * cf);
	color *= rrot;
	...
	// love
	float resonance = pow(sin(cr.x * 11.8) * sin(cr.y * 13.72) * (0.382 + 0.618 * u_intensity), 3.0);
    ...
    for(int i=0; i<7; i++){
        ...
        col += vec3(drawBraille(pos, base, r, brailleCodes[i])) * pow(0.618 + 0.118 * u_volume_smooth / (0.118 + punch), 3.0);
		...
		// field
		col /= 0.382 + 0.618 * (1.0 / (map_field(color, u_volume_smooth) + 0.382)) * (0.1 + u_volume_smooth);
		// ridge
		col /= clamp(ridgeFBM(base), 0.118, 0.228);
		
		// thunder
		float fade = pow(0.618, fi);
	...
	// HH
	finalColor /= 0.118 + (1.0 - smoothstep(0.0, 1.0, sym)) * 0.618;
	...
	vec3 emission = color * abs(sym) * (u_volume_smooth * 0.382 + 0.618) * 6.18;
	vec3 currentFrame = mix(emission, past, 0.382); 
