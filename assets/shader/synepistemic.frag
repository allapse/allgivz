...
void main() {
    vec2 uv = v_uv;
    vec2 pos = (uv - 0.5) * 17.0;
	float punch = pow(u_intensity * u_complexity * u_speed * u_peak, 3.0);
	float freeze = 1.0 - (u_intensity + u_complexity + u_speed + u_peak) / 4.0;
	float bigZ = v_z * 7.0 * freeze;
	
	if(u_darkGlow > 0.5) {
		bigZ /= u_volume;
	}

    float wave = pow(0.5 + 0.5 * sin(u_time + pos.x * u_complexity) / (0.05 + 0.95 * bigZ), 3.0);
    wave *= u_volume_smooth;

    float sym = pow(sin(u_time + pos.x * u_complexity + bigZ) / (0.05 + 0.95 * cos(pos.y * u_intensity - bigZ)), 3.0);

    float motion = pow(0.3 + 0.7 * sin(u_time + length(pos) * u_speed + bigZ), 3.0);

    vec3 baseColor   = vec3(0.1, 0.2, 0.4);
    vec3 accentColor = vec3(1.0, 0.6, 0.2);
    vec3 peakColor   = vec3(0.2, 0.8, 1.0);

    vec3 color = mix(baseColor, accentColor, wave);
    color = mix(color, peakColor, u_peak);
	
    color *= sym * motion * max(0.3, u_intensity);
	...
	if(u_darkGlow > 0.5) {
		past += vec3(sin(u_intensity), u_complexity, cos(u_speed));
